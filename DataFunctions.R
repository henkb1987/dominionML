...
get.n.players <- function(game.data){
  return(length(which(strsplit(game.data,"")[[1]]=="*")))
}
...
get.game.states <- function(game.data){
  ...
  for(p in 1:n.players){
    ...
    game.states[1:n.turns,,"supply"] <- game.states[,,"supply"] - apply(buy.matrix[1:n.turns,,p], 2, cumsum)
    # only opponent players: adjust opponent inventory
    if(p != perspective){
      game.states[1:n.turns,,"opp.inventory"] <- game.states[1:n.turns,,"opp.inventory"] + apply(buy.matrix[1:n.turns,,p], 2, cumsum)
    } else {
      # only perspective player: adjust my inventory
      game.states[1:n.turns,,"my.inventory"] <- apply(buy.matrix[1:n.turns,,p], 2, cumsum)
...
get.game.states.inoutput <- function(games){
  load("card.info")
  inputs <- matrix(NA, 1e3, 3 * 173)
  outputs <- matrix(NA, 1e3, 173)
  counter <- 1
  
  for(i in 1:length(games)){
    cat("Processing game states for game",i,"...\n")
    game.states <- get.game.states(games[i])
    n.turns <- dim(game.states)[1]
    if(counter + n.turns + 5 > nrow(inputs)){
      cat("Increasing input/output matrices...\n")
      inputs <- rbind(inputs, matrix(NA, 1e3, 3 * 173))
      outputs <- rbind(outputs, matrix(NA, 1e3, 173))
    }
    inputs[counter:(counter + n.turns - 1), ] <- cbind(game.states[,,"supply"], game.states[,,"my.inventory"], game.states[,,"opp.inventory"] / (get.n.players(games[i]) - 1))
    outputs[counter:(counter + n.turns - 1), ] <- game.states[,,"my.inventory"]
    counter <- counter + n.turns
  }
  inputs <- inputs[complete.cases(inputs), ]
  outputs <- inputs[complete.cases(outputs), ]
